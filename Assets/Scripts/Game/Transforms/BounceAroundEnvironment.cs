...
public class BounceAroundEnvironment : MonoBehaviour, Types.IRoom_EnemyObject
{
	[Header("Tweakables")]
	...
	public void OnReset()
	{
		// Quite like these objects not resetting, player just has to duck and dive out of the way!
	}
	...
	private void FixedUpdate()
	{
		...
		Vector3 vBounce = MathUtil.GetReflectionVectorFromBoundsCheck(transform.position, m_gcCollider.radius);
		...
		// Now move...
		Vector3 vNewPos = MathUtil.GetBoundsCheckedVector(transform.position + ((m_vTrajectory * m_fMovementSpeed) * TimerManager.fFixedDeltaTime));
		vNewPos.z = 0.0f;
	...
	private void OnCollisionEnter2D(Collision2D col)
	{
		...
		// GNTODO: try raycasting back along the trajectory to find the cleanest impact...?
